...
<template>
  <div style="overflow: auto; height: 86vh;">
      <div>
        <!-- <img src="~assets/sms-banner.jpg" width="100%"> -->
      ...
      <div class="q-pa-md row">
            ...
            <div class="items-start q-pa-md row">
                ...
                <div class="col-sm-6 col-xs-6 q-pa-sm">
                  <div>
                    ...
                    rounded  
                    color="green" 
                    icon-right="fa-solid fa-circle-plus" 
                    label="Create Message Blast" 
                    @click="openAddSMSBlastForm()" />
                  </div>
                </div>
            </div> 
        </div>
        ...
        <div class="col-md-3">
          <q-card
            ...
            >
            <q-card-section>
              <div class="text-h1 text-center">{{ blasts.length }}</div>
              <div class="text-subtitle2">TOTAL SMS BLAST MADE</div>
            </q-card-section>
            </q-card>
        </div>
      </div>
          ...
          <div class="q-pa-md" style="padding-top:20px">
              ...
              <!-- <q-dialog v-model="persistent" persistent transition-show="scale" transition-hide="scale">
                ...
                <q-dialog v-model="addSMSBlastForm" transition-show="scale" transition-hide="scale">
                  <q-card  style="min-width: 600px">
                    <q-card-section class="bg-green text-white">
                      <div class="text-h6">Create SMS Blast</div>
                    </q-card-section>
                    <q-separator />
                    <q-card-section class="q-pt-md q-ma-md q-pt-none">
                      <q-form
                        class="q-gutter-md"
                      >
                     <div class="text-center text-h6"> Soon to be added.... </div>
                      <!-- <q-select 
...
export default defineComponent({
  name: 'SMS Blast Page',
  computed: {
    ...mapGetters('SMSBlastManagement', {
      blasts: 'GET_ALL_SMS_BLASTS',
    }),
  ...
  data: () => ({
      ...
      columns : [
          ...
          { name: 'member_numbers', label: 'Recipients\' Numbers',  field: row => row.member_numbers,align: 'left', sortable: true },
          { name: 'message_content', label: 'Message Content',  field: row => row.message_content,align: 'left', sortable: true },
          { name: 'created_by', label: 'SMS Blast Created By',  field: row => row.creator.first_name + ' ' +row.creator.middle_name + ' ' +row.creator.last_name ,align: 'left', sortable: true },
          { name: 'created_at', label: 'Initiated At',  field: row => moment(row.created_at).format('MMMM Do YYYY'),align: 'left', sortable: true },
          // {
          //   name: 'active',
          //   required: true,
          //   label: 'ACTION',
  ...
  methods: {
    ...
    async openAddSMSBlastForm(){
      this.addSMSBlastForm = true;
    },

    async addBlast() {
      this.loading = true;
      this.addBRGYSMSBlast({
    ...
    async refresh(){
      await this.getBRGYSMSBlasts;
      await this.getResidents;
      this.rows = this.blasts;
      console.log("Test")
