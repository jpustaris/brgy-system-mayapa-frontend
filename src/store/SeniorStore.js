...
export default {
    ...
    actions:{
        async getSeniors(context) {
            ...
            await this.$axios.get('/api/get-seniors')
                .then(response => {
        ...
        async addSingleSenior(context, payload) {
            context.commit("SET_LOADING", true)
            await this.$axios.post('/api/seniors', {
                name: payload.name,
              }).then(response => {
        ...
        async editSingleSenior(context, payload) {
            await this.$axios.put('/api/seniors/'+ payload.selected_id , {
        ...
        async disableSingleSenior(context, payload) {
            await this.$axios.delete('/api/seniors/'+ payload.selected_id).then(response => {
                    console.log(response.data)
                    context.commit("SET_ALL_SENIORS", response.data.data)
            }).catch((error) => {
                    console.log(error)
